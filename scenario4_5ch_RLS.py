"""
SCENARIO 4 -- 5-channel reference (V2, V5, V8, RA, LA), RLS regression.
...
REF_LEADS = ['V2', 'V5', 'V8', 'RA', 'LA']

print("=" * 70)
print(f"{TAG}: 5-channel reference, RLS maternal-template regression")
print("=" * 70)
...
# 2. Maternal R-peak consensus (Pan-Tompkins on V2/V5/V8, as in the 3-channel scripts)
...
# 4. Windowed-fit PCA -> maternal components M1, M2 (drawn from a 5-lead basis)
PC = fc.run_pca_windowed(P, W, n_retain=2)
print(f"Explained variance PC1+PC2: {PC['cumulative_variance'][1]*100:.2f}%")

# 5. RLS maternal-template regression  <-- scenario-specific step
REG = fc.fit_maternal_template_rls(P, PC)
print(f"  lambda={REG['lam']}  delta={REG['delta']}")
for lead in fc.ABDO_LEADS:
    w0 = REG['results'][lead]['w_ols']
    wf = REG['results'][lead]['weights'][-1]
    print(f"  {lead}: OLS warm-start=[{w0[0]:+.4f},{w0[1]:+.4f}]  final RLS=[{wf[0]:+.4f},{wf[1]:+.4f}]")
